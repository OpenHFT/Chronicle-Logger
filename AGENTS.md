...
For static analysis and coverage (see `QUALITY_PLAYBOOK.md` for details):

* From the repo root, run `mvn -P quality clean verify` to apply the shared Checkstyle and SpotBugs rules from `root-parent-pom` across all modules.
* From the repo root, run `mvn -P sonar clean verify` to generate JaCoCo coverage and SonarCloud inputs.
* For a Chronicle-Logger focused quality pass on Java 11+, run:

```bash
mvn -q -pl :chronicle-logger -am clean verify
```

This auto-activates the `quality` profile in the Chronicle-Logger POMs so Checkstyle and SpotBugs run as gating checks.
